    ...
    <div class="px-6 py-4 border-b border-base-300 space-y-3">
      ...
      <div class="relative">
        ...
        <input
          ...
          placeholder={t("agent.modelSelect.searchModelPlaceholder")}
          class="w-full pl-10 pr-4 py-2 border border-[var(--hairline)] bg-base-300 rounded-md focus:border-[var(--field-border-hover)]"
        />
        {#if searchQuery}
          <button
            onclick={clearSearch}
      ...
      <div class="flex items-center justify-between gap-3">
        <div class="text-xs text-base-content/70">
          {#if isLoadingModels}
            {t("agent.modelSelect.loadingModels")}
          {:else}
            {t("agent.modelSelect.modelCount", { count: filteredModelsResult.length })}
          {/if}
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <Select
            ...
            options={[
              { value: "all", label: t("agent.modelSelect.allProviders") },
    ...
    <div class="flex-1 overflow-y-auto">
      {#if filteredModelsResult.length === 0}
        ...
        >
          ...
          <p class="text-lg">{t("agent.modelSelect.noMatchingModels")}</p>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      {#if model.support_image}
                        <EyeIcon
                          size={14}
                          class="text-info"
                          aria-label={t("agent.modelSelect.supportsImageGeneration")}
                        />
                      {/if}

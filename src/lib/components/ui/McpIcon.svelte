<script lang="ts">
  /**
   * Official Model Context Protocol mark, shaped like a Lucide icon (`size` +
   * `class`, `currentColor` fill) so it drops into the same slots — including
   * the `typeof Icon` props of MenuButton and the settings nav.
   */
  import type { ClassValue } from "svelte/elements";

  // Mirrors Lucide's prop types so this stays assignable to `typeof Icon`.
  interface Props {
    size?: number | string;

...
export async function getMessages(
  ...
  const list = await apiCall<any[]>("message_list", {
    chatId: chatId,
    limit,
    offset,
  });
...
export async function updateMessage(
  ...
  const r = await apiCall<any>("message_update", {
    messageId: messageId,
    content,
  });
  return r;
...
export async function regenerateAssistantMessageStream(
  ...
  await apiCall<void>("message_assistant_regenerate_stream", {
    messageId: messageId,
  });
}
 ...
 */
export async function resendUserMessageStream(
  messageId: UUID,
  content?: string,
): Promise<void> {
 ...
 * 流式执行工具调用
 */
export async function executeToolCallsStream(
  messageId: string,
  toolCallIds: string[],
): Promise<void> {

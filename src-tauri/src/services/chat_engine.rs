...
use hand_ai_model::{
    self as model, AssistantMessageEvent, Client, Context, ImageContent, Message, StopReason,
    TextContent, Tool, Usage, UserContentBlock, UserMessage,
};

use crate::models::llm_types::LlmMessageRole;
use crate::models::AppError;
// `ChatOptions` / `ChatTool` / `HydratedAttachment` and the model-resolution
// helpers (`resolve_model` / `resolve_model_template` / `build_stream_options`)
// moved to `model_runtime` so they outlive the chat backend. Re-exported here
// for the soon-to-be-removed chat services (`message`, `session`) that still
// reach them via `chat_engine::`.
use crate::services::model_runtime::{build_stream_options, resolve_model, resolve_model_template};
pub use crate::services::model_runtime::{ChatOptions, ChatTool, HydratedAttachment};
...
fn chat_user_message(
    ...
    if !msg.content.is_empty() {
        blocks.push(UserContentBlock::Text(TextContent::new(
            msg.content.clone(),
        )));
    }
...
fn tool_calls_from_assistant_message(
...
mod tests {
    ...
    fn multiple_system_messages_concatenated_with_blank_line() {
        let messages = vec![system_msg("first"), user_msg("hi"), system_msg("second")];
        let ctx = messages_to_context(&messages, "openai", "gpt-4o", &ChatOptions::default())
            .expect("translation");
    ...
    fn terminal_chunk_aggregates_tool_calls_from_done_event() {
        ...
        let args_a: serde_json::Value =
            serde_json::from_str(&calls[0].arguments).expect("call_a arguments round-trip");
        assert_eq!(args_a, serde_json::json!({"q": "x"}));
        let args_b: serde_json::Value =
            serde_json::from_str(&calls[1].arguments).expect("call_b arguments round-trip");

...
/// 跟踪禁用面板是否可见
static DISABLE_PANEL_VISIBLE: AtomicBool = AtomicBool::new(false);
/// 跟踪鼠标是否在面板内部
static MOUSE_INSIDE_PANEL: AtomicBool = AtomicBool::new(false);

const PANEL_LABEL: &str = "selection_settings_disable";

tauri_panel! {
    panel!(SelectionSettingsDisablePanel {
        config: {
    ...
    panel_event!(SelectionSettingsDisableEventHandler {
...
pub fn init_panel(app_handle: &AppHandle) {
    ...
    let panel = window.to_panel::<SelectionSettingsDisablePanel>().unwrap();
    ...
    let handler = SelectionSettingsDisableEventHandler::new();
    handler.on_mouse_entered(move |_event| {
        MOUSE_INSIDE_PANEL.store(true, Ordering::Relaxed);
        tracing::debug!("Mouse entered settings disable panel");
    });
    handler.on_mouse_exited(move |_event| {
        MOUSE_INSIDE_PANEL.store(false, Ordering::Relaxed);
        tracing::debug!("Mouse exited settings disable panel");
    });
    handler.window_did_become_key(move |_notification| {
        tracing::debug!("Settings disable panel became key window");
    });
    handler.window_did_resign_key(move |_| {
        tracing::debug!("Settings disable panel resigned from key window");
    });
...
pub fn show_panel(handle: &AppHandle, x: f64, y: f64) {
    DISABLE_PANEL_VISIBLE.store(true, Ordering::Relaxed);
...
pub fn hide_panel(handle: &AppHandle) {
    DISABLE_PANEL_VISIBLE.store(false, Ordering::Relaxed);
    let handle_clone = handle.clone();
    let _ = handle.run_on_main_thread(move || {
        if let Some(window) = handle_clone.get_webview_window(PANEL_LABEL.into()) {
...
pub fn is_panel_visible() -> bool {
    DISABLE_PANEL_VISIBLE.load(Ordering::Relaxed)
}

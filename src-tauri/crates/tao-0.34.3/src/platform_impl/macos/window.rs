...
use objc2_app_kit::{
  ...
  NSEventModifierFlags, NSEventSubtype, NSEventType, NSPanel, NSRequestUserAttentionType, NSScreen,
  NSView, NSWindow, NSWindowButton, NSWindowCollectionBehavior, NSWindowFullScreenButton,
...
fn create_window(
  ...
  unsafe {
    ...
    let is_nonactivating_panel =
      !attrs.focusable && !attrs.decorations && attrs.always_on_top && attrs.transparent;
    ...
    if is_nonactivating_panel {
      masks |= NSWindowStyleMask::NonactivatingPanel;
    }

    let window_class = if is_nonactivating_panel {
      PANEL_CLASS.0
    } else {
      WINDOW_CLASS.0
    };
    ...
    if is_nonactivating_panel {
      let ns_panel: &NSPanel = &*(ns_window_ptr as *mut NSPanel);
...
lazy_static! {
  ...
  static ref PANEL_CLASS: WindowClass = unsafe {
    let window_superclass = class!(NSPanel);
    let mut decl = ClassDecl::new(
      CStr::from_bytes_with_nul(b"TaoPanel\0").unwrap(),
      window_superclass,
    )
    .unwrap();
    decl.add_method(
...
extern "C" fn accepts_first_mouse(_this: &Object, _: Sel, _event: &NSEvent) -> Bool {
  Bool::YES
}

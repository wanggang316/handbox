...
- **工具**（自 milestone M2 起，`services/agent_tools.rs`；⚠ 此 legacy 工具集随后端运行时退役而失活——自 coding-agent-embed 计划 M1 起活跃 run 改用 coding-agent `create_default_tools`，M4 进一步删除了出站 `web_fetch` 工具及其全套 SSRF 守卫与测试。`build_tools`/`make_skill_tool`/`make_read_file_tool`/`make_list_directory_tool` 仍为 `pub fn` 但已无生产调用方（已知 tech-debt，待清理）；`resolve_in_sandbox` 沙箱解析器**保留**且仍被 `agent_permission` 复用）：内置只读 FS（`read_file` / `list_directory`）+ 出站 `web_fetch`，由 `build_tools(enabled, working_dir)` 单点装配。
...
- **coding-agent 驱动路径**（自 coding-agent-embed 计划 M1 起；⚠ legacy `agent_runtime` 路径已于 coding-agent-embed M4 退役，本路径现为 Agent 模式**唯一** run 路径）：`agent_run_stream` 现经 hand-ai `hand-coding-agent::AgentSession` 驱动一轮 prompt，**前端契约逐字不变**（事件名 / payload 形状 / closed-once / 错误分型）。delete-time abort 重接线到 `coding_agent_runtime::abort_run`（同步、对无活跃 run 是干净 no-op，并 fail-close 该 session 上 parked 的审批），`agent_session_delete`/`agent_project_delete` 已移除其 Tauri 注入的 `State<AgentRuntime>`（前端 invoke 透明）。迁移自 pre-M4 SQLite 的会话其 `enabled_tools` 经 `select_enabled_tools` 内非破坏式运行时 remap（`read_file→read`/`list_directory→ls`；`web_fetch`/`skill`/未知名原样透传交由内置过滤丢弃，绝不全放行，SQLite 列从不重写）。forcedSkills 在退役后经 `agent_run_stream` 重接线：本回合 forced skill 名针对**有效集**（`SkillService.discover ∖ skills.disabled`）解析、body 逐字注入 `config.system_prompt`（语义详见第 114 条；disabled 优先于 forced、空结果保持 `None` 不覆盖默认提示词）。
  - **构造**（`services/coding_agent_session.rs`）：`build_agent_session(&HandBoxAgentSessionConfig)` 复用 `chat_engine::{resolve_model, build_stream_options}` 装配 `model::Model` + `SimpleStreamOptions`（plaintext key **仅**入 `stream_options.base.api_key`，绝不写 auth.json / env / keyring）；`base_dir` 钉到 Tauri `app_data_dir`（持久化留在 app sandbox，非 `~/.hand`）；本 milestone `no_session = true`（纯内存，JSONL 持久化是 M3）。`select_enabled_tools(cwd, enabled)` 从 `create_default_tools` 的 7 个内置工具（注册名 `read`/`write`/`edit`/`bash`/`grep`/`find`/`ls`）按名门控——空列表注册零工具（沿用 legacy「未列即未启用」约定），未知名 warn-log 后丢弃不致构造失败。
  - **驱动 + 事件映射**（`services/coding_agent_runtime.rs`，drive 策略 A：subscribe 直驱）：`drive_agent_run` 非阻塞 spawn 后台任务 `send_message`；`map_session_event` 把 `AgentSessionEvent::Agent(inner)` 逐字 `Forward` 到 `agent_stream_event`（内层 `AgentEvent` 与 legacy 同型、serde 形状一致），run-level `Err` 经 `sanitize_coding_agent_error`（复用 legacy `AgentError` 分型，AUTH/RATE_LIMIT/NETWORK/INTERNAL）发为 `agent_stream_error` **在唯一 closed emit site 之前**，closed 恰好一次。
  - **新增第四通道 `agent_session_lifecycle`**：compaction（`CompactionStart`/`CompactionEnd`，summary 有意不渲染）与 `SessionInfoChanged` 经此独立通道发为 `{ sessionId, kind, .. }`，**绝不进 `agent_stream_event` reducer**，故 closed-once 不受影响；前端据 `kind` 渲染「整理上下文中」指示并即时更新侧栏标题（单向回调 `agentRun → agentSession`）。
  - **in-band key-scrub 安全**：上游 proxy 在非 2xx 时把原始 provider body 写入 `AssistantMessage.errorMessage`（可回显 key 片段），该消息走 `Ok` 流故**不经** run-level sanitizer——`redact_inband_error_messages` 在映射层把 `stopReason=="error"` 消息的 `errorMessage` 替换为通用常量，仅此一字段，正文 `content` 逐字保留（不抹已流出文本）。
  - **steer/abort 注册表**：进程级 `session_id → RunControl` 注册表（`run_controls`，与 `commands/agent_run.rs` 的 `active_coding_runs` 并发去重集互补）持有从 `AgentSession` clone 出的 cancel / steering handle，`drive_agent_run` 入口注册、closed emit site 注销，恰好 bracket run 生命周期；`abort_run` 翻转 cancel token、`steer_run` 把 user `Message` push 进 in-flight 轮的 steering 队列（并入当前轮、不进 follow-up）；空白文本 / 无活跃 run 均为干净 no-op。
  - **沙箱 + 危险工具门禁**（`services/agent_permission.rs`，`before_tool_call` Tier-1 Extension 链，host 按注册序分发、首个 `Cancel` 胜出）：`SandboxExtension` 对 `read`/`ls` 复用 `agent_tools::resolve_in_sandbox` 把 `path` 参数限制在 `working_dir` 内（拒绝越界绝对路径 / `~` 展开 / `..`，reason 不泄露路径）；`DangerousDenyExtension`（M1 stub）对 `write`/`edit`/`bash` 无条件 `Cancel`（M2 以审批扩展替换该 stub 的单点 `decide()`）。
...
### 3.5 定时任务（Scheduled Jobs，独立子系统，自 scheduled-jobs 计划 M1 起）

定时任务子系统按 cron 表达式自动执行三类目标（artifact / prompt / agent），与 Chat/Agent 子系统数据隔离（独立的 `jobs` / `job_executions` 表），经侧栏全局区入口 `/jobs` 进入。

- **存储**（迁移 `049_create_jobs.sql` / `050_create_job_executions.sql` / `051_add_job_robustness_columns.sql`）：`jobs` 表含 cron/timezone/target_kind/target_config(JSON)/enabled/last_run_at/next_run_at/last_status/`run_count`/`failure_count` + 索引 `idx_jobs_enabled_next`；051 追加健壮性列 `exec_timeout_secs`(0=无限) / `max_retries`(0=不重试) / `retry_delay_secs`(默认 60)，均 NOT NULL + DEFAULT 回填旧行（SQLite ALTER 只能 ADD COLUMN）。`job_executions` 表一执行一行，`job_id` FK `ON DELETE CASCADE`；`JobTarget` 为 serde tagged enum（`kind=artifact|prompt|agent`，per-kind config）。
- **运行时分层（核心契约）**：`JobScheduler`（`services/job_scheduler.rs`）= `tauri::async_runtime::spawn` 的固定 30s「work→sleep」tick 循环，**DB 为单一事实源**（每 tick 重读 due 列表），at-most-once 由「dispatch 前先推进 `next_run_at`」保证；重入由 `JobExecutor` 上的共享 `Arc<Mutex<HashSet<String>>>` in-flight 集 + RAII `InFlightGuard`（`try_lock` 快路径 + contended 时 detached 兜底，Drop 永不阻塞 worker）守护；每个 job 以独立 detached `tokio::spawn` 运行（慢/panic 的 target 既不阻塞也不杀循环）。`JobExecutor`（`services/job_executor.rs`）拥有 in-flight 集；scheduler clone 同一 executor Arc 与之共享。**`next_run_at` 所有权**：scheduler 推进（DB + 内存 job 双写）、executor 经 `update_after_run` 原样保留——杜绝双触发/重置刚触发的 occurrence。
- **执行信封（一 trigger = 一行，自 M1 叠加至 M4）**：`execute()` = `try_claim`（调用方持 guard 横跨整个信封）→ `insert_running`(attempt=1) → 重试循环 `{ dispatch_with_timeout; success 或耗尽 attempts 即 break；否则 backoff sleep + 重检 job 仍存在 }` → `finalize`（同一行原地翻到终态，写最终 attempt）→ `update_after_run` → 失败通知 → `prune_to`（FIFO 至 N=100，置于 finalize 后，永不删 running 行）→ `emit_executed`。三类 target（artifact 子进程 `kill_on_drop` / prompt future-drop / agent cooperative `abort`）均被 timeout 与 retry 正确包裹；in-flight guard 横跨**整个信封**（含每次 backoff），mid-backoff 的 tick 被跳过而非开第二行。
- **计数语义**：`run_count` 累计（每信封 +1）；`failure_count` 为**连续失败**计数——scheduled 成功 reset、scheduled 失败/timeout +1、manual 永不触碰（`failure_count_update(trigger,status)` 纯函数）。失败通知（`tauri-plugin-notification`）在 `failure_count == 3`（`should_notify_failure`，仅 Schedule）触发一次，4/5 次静默，成功 reset 后自然 re-arm；权威计数从持久化行回读（非内存快照）；无权限时优雅降级（记日志、吞错、不阻断执行）。
- **timeout / retry**：`exec_timeout_secs>0` 是单一上限、置于 artifact 内建 30s 之外（先触发并记 `status='timeout'` 而非 `failed`）；`0=无限`。backoff = `retry_delay_secs * 2^(attempt-1)`（base 2，指数 clamp 防溢出，`retry_delay_secs<=0 → ZERO` 无忙等）；sleep 在 task 内、shutdown 即丢弃（不跨重启 replay）；mid-envelope 删除（job 被级联删）干净中止为 `Err(not_found)`，不 finalize 已不存在的行。
...
生成式 UI 让模型输出一份扁平 `Spec`（`root` + `elements` 字典），客户端用一套**受控的展示组件 catalog**把它渲染成 UI，而非自由 HTML。基于 `@json-render/core` + `@json-render/svelte` 的三层（Catalog / Agent / Render）。代码位于 `src/lib/components/chat/renderers/jsonui/`，是 chat 路径**唯一**的结构化渲染机制——早期 PoC 的「信封方案」（`renderers/` 的 envelope/registry/TranslationCard，单一 `translation` renderer）因始终无生产发出方、已整条移除，由 json-render 的通用组件组合替代（含等价的翻译卡，见组件画廊 Spec A）。M1（catalog-expansion）只交付 catalog + 渲染 + 校验 + 组件画廊；Agent 级开关、prompt 注入与流式接入属 M2（agent-integration），尚未实现。
...
- **Agent 级开关 + 注入 + 流式渲染（generative-ui M2）**：让任意 chat-Agent 经一个开关输出 spec。范围**只作用于 chat-Agent**（`agents` 表 → 由 Agent 创建的 chat `sessions` → 发消息），**不碰** coding-agent 子系统（`agent_sessions`）。
  - **`generative_ui` 字段**（migration `052`，`agents` + `sessions` 各加可空 `INTEGER`）：`Option<bool>`，NULL（旧行）经 `try_get::<Option<bool>>` 解码为 `None` ≡「关」。两条 wire-key 习惯**有意不对称**——响应/round-trip 的 `Agent`/`Session`（`rename_all="camelCase"`）是 `generativeUi`（小写 i，由测试 `*_wire_key_is_camel_case` 锁定）；而 create 请求 `commands/agent.rs::AgentCreateRequest`（**无** `rename_all`）用 snake `generative_ui`（前端 `api/agent.ts` 显式映射），update 走 camel fieldName `"generativeUi"`。`create_session_from_agent` 把 agent 值**快照**进 session；该列是 **write-once**——`update_session` 的 SET 子句**刻意排除**它（与 `project_id` 同），避免无关编辑清空快照。表单关开关的 false **必须**走显式布尔 diff（`(a ?? false) !== (b ?? false)`），不能被 truthy 判断吞掉。
  - **固化 prompt 注入（bake-and-drift 工作流）**：前端纯函数 `buildGenerativeUiPrompt(uiCatalog)`（`jsonui/prompt.ts`，确定性、whole-spec 导向、内嵌 resolveSpec 可解析的示例、无 JSONL/patch 措辞）经 `scripts/gen-generative-ui-prompt.ts`（npm `gen:gui-prompt`）烤进**已提交**资源 `src-tauri/resources/generative-ui-prompt.txt`；后端 `include_str!` 编译期嵌入。一条 vitest **drift 测试**逐字节比对资源与 builder 输出——catalog 改了却没重跑生成脚本即 CI 失败。
  - **注入点（单一 choke point）**：`services/message.rs` 的纯函数 `build_system_messages(system_prompt, generative_ui)` 在 `build_message_request`（send / resend / regenerate / tool-call 续聊**全部 5 条发送路径**的唯一汇聚点）里，仅当 `generative_ui == Some(true)`（严格匹配，NULL/false 不注入）追加一条独立 System 段。注入是**单回合、不持久化**（绝不写回 `session.system_prompt`）、**与用户 system_prompt 共存**、每次重建故**不堆叠**。
  - **流式渲染**（`MessageAssistant.svelte`）：`specHit` 在流式期间也跑 `resolveSpec`（由 `looksLikeStreamingSpec` 门控以省下普通 markdown 每 token 的解析）。未闭合 → `resolveSpec` 返回 null → 显「结构化结果生成中…」占位（`isStreamingStructured` 由 `looksLikeStreamingSpec` 单独门控）；闭合且校验通过 → `<Renderer loading={isStreaming}>`；截断/未注册组件 → null → 回退 markdown（无卡死 spinner）。分流优先级 `specHit > markdown`，既有 markdown/代码/KaTeX/工具卡片/图片/复制/收藏/重生成分支字节不动。

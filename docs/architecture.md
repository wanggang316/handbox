...
- **coding-agent 驱动路径**（自 coding-agent-embed 计划 M1 起，与上述 legacy `agent_runtime` 路径**并存**，legacy 待 M4 退役）：`agent_run_stream` 现经 hand-ai `hand-coding-agent::AgentSession` 驱动一轮 prompt，**前端契约逐字不变**（事件名 / payload 形状 / closed-once / 错误分型）。

...
- **Agent 级开关 + 注入 + 流式渲染（generative-ui M2）**：让任意 chat-Agent 经一个开关输出 spec。范围**只作用于 chat-Agent**（`agents` 表 → 由 Agent 创建的 chat `sessions` → 发消息），**不碰** coding-agent 子系统（`agent_sessions`）。
  ...
  - **流式渲染**（`MessageAssistant.svelte`）：`specHit` 在流式期间也跑 `resolveSpec`（由 `looksLikeStreamingSpec` 门控以省下普通 markdown 每 token 的解析）。未闭合 → `resolveSpec` 返回 null → 显「结构化结果生成中…」占位（与 `__render` 信封共用）；闭合且校验通过 → `<Renderer loading={isStreaming}>`；截断/未注册组件 → null → 回退 markdown（无卡死 spinner）。分流优先级 `specHit > cardHit(信封) > markdown` 不变，既有 markdown/代码/KaTeX/工具卡片/图片/复制/收藏/重生成分支字节不动。

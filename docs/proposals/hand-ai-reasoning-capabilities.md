...
## 10. Catalog 准确性 — Cerebras 幽灵模型（独立于 P1-P4）

这是一个**数据准确性**问题，不是 API shape，单独记录。

**现象**：2026-06-05 HandBox UI smoke 中，在 Cerebras 上选 `qwen-3-235b-a22b-instruct-2507` 发消息，Cerebras API 直接返回：

```json
{"message":"Model qwen-3-235b-a22b-instruct-2507 does not exist or you do not have access to it.",
 "type":"not_found_error","param":"model","code":"model_not_found"}
```

**核对**（用真实 key 拉 Cerebras live `/v1/models`）：

| hand-ai catalog 列出 | Cerebras live `/v1/models` 实际可用 |
|---|---|
| `gpt-oss-120b` | ✅ `gpt-oss-120b` |
| `qwen-3-235b-a22b-instruct-2507` | ❌ 不在列表 |
| `zai-glm-4.7` | ✅ `zai-glm-4.7` |

即 hand-ai 的 `cerebras` catalog 里 `qwen-3-235b-a22b-instruct-2507` 这条，在该账号的 Cerebras live API 上不存在。

**对比 DeepSeek（无问题）**：同样核对 DeepSeek，catalog 的 `deepseek-v4-flash` / `deepseek-v4-pro` 与 live `/v1/models` 完全一致，无漂移。所以这是 Cerebras 单条目问题，不是普遍现象。

**两种可能**，请 hand-ai 侧裁量：

1. catalog 过时 —— Cerebras 已下线 `qwen-3-235b-a22b-instruct-2507`，应从 `models.json` 的 `cerebras` 段删除。
2. 该模型需要特殊 tier / 额度，普通账号拿不到 —— 静态 catalog 无法表达 per-account 可用性，那这条是 catalog 的固有局限，hand-ai 可以选择保留（标注需特殊访问）或删除。

**复核命令**（hand-ai 侧可用任意有效 Cerebras key 复现）：
...
**HandBox 架构原则（已与用户确认 2026-06-05）**：HandBox 的模型列表**完全依赖 hand-ai catalog**，不引入任何 HandBox 侧的 live `/v1/models` 轮询或交集过滤。这是有意的单一真理源设计——HandBox 保持薄，模型目录的正确性由 hand-ai 负责。

由此推出对 hand-ai 的要求：

- catalog 应只收录**该 provider 普遍可用**的模型；已下线 / 仅特殊 tier 可访问的条目应删除或显式标注。
- HandBox 接受「catalog 中的模型在某账号下可能因无权限而在对话时报 `model_not_found`」这一固有 gap——这是完全依赖静态 catalog 的代价，HandBox 不会用 live 过滤去兜底。

用户当前对本例的手动规避：在模型设置页把 `qwen-3-235b-a22b-instruct-2507` disable 即可，等 hand-ai 修 catalog 后随下次同步消失。
